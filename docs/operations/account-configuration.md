...
- Anam: `ANAM_API_KEY` plus `ANAM_PERSONA_ID` are populated locally; local and production session-token checks pass.
...
3. Keep `ANAM_PERSONA_ID` synchronized between `avatar/.env` and Vercel secrets.
