...
- Anam: `ANAM_API_KEY` plus `ANAM_AVATAR_ID` are populated locally; local and production session-token checks pass.
...
3. Keep `ANAM_AVATAR_ID` synchronized between `avatar/.env` and Vercel secrets.
